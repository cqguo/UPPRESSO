...
The IdP should encrypt random user identities in storage and decrypt them only in memory.

If $ID_U$ is leaked to RPs, this user's privacy against RP-based identity linkage is broken;
 however, in this case, \usso\ still provides secure SSO services, providing that in Step 4.2 an (honest) RP accepts only identity tokens with \emph{matching} $PID_{RP}$,
 because adversaries cannot deceive any honest RP other than the designated one to accept a token (i.e., no $PID_{RP} collision$) in this case.
We prove it in Appendix \ref{proof-rp-collision}.
...
We plan to adopt a quantum-resistant public-key algorithm to sign identity tokens and RP certificates,
 ...
 to investigate if they support RP designation as discussed in Section \ref{analysis-security} (i.e., no collision exists in the blinded inputs of evaluated pseudo-random functions).
