...
%    \begin{tabular}{|c|c|c|}
   ...
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$ is the $j$-th RP's unique identity, which is publicly known; $r \in \mathbb{Z}_n$ is known to \emph{nobody}.} \\ \hline
    ...
    {$PID_{RP_j}^k$} & {$PID_{RP} = [t]{ID_{RP}} = [tr]G$ is the $j$-th RP's pseudo-identity, in the user's $k$-th login to this RP.} \\ \hline
    {$PID_{U,j}^k$} & {$PID_U = [{ID_U}]{PID_{RP}} = [utr]G$ is the user's pseudo-identity, in the user's $k$-th login to the $j$-th RP.} \\ \hline
...
\noindent {\bf $\boldsymbol{ID_{\boldsymbol{RP}}}$-$\boldsymbol{PID_{\boldsymbol{RP}}}$ Transformation.} In each login, a user selects a random number $t \in \mathbb{Z}_n$ as the trapdoor to calculate $PID_{RP}$.
\begin{equation}
...
A COTS browser without extensions or plug-ins works as the user agent in \usso,
 and the user-agent functions are implemented using web scripts in browsers.
 Two scripts are downloaded from the IdP and the visited RP, respectively,
        and responsible for communications with the origin web servers.
It is worth noting that a script downloaded from an honest entity is considered \emph{honest}.

The IdP script is necessary because the RP script could leak its origin to the IdP web server due to the \emph{automatic} inclusion of an HTTP \verb+referer+ header in all HTTP requests it sends.
Besides, the IdP script is trusted to interact with the users for attribute authorization,
  while the RP (and its script) might be malicious.
...
\item
The IdP generates random $r \in \mathbb{Z}_n$
        and assigns a \emph{unique} point $[r]G$ on $\mathbb{E}$ to $ID_{RP}$.
Thus, $ID_{RP} = [r]G$ but $r$ is known to \emph{nobody}
...
\usso\ requires $ID_U$ is known \emph{only} to the IdP,
    and $ID_U$ is used by the IdP \emph{internally},
...
For example, a user's identity may be generated and always restored by hashing her username concatenated with the IdP's private key, 
    and $ID_U$ is \emph{only} used to calculate $PID_{U}$ when the IdP is signing a token binding $PID_{U}$.
Thus, $ID_U$ is protected almost the same as the IdP's private key.

\vspace{1mm}
\noindent\textbf{SSO Login.} A login flow %is typically launched through a browser,
...
replies with $Cert_{RP}$ and the scope of the requested user attributes. The reply is then forwarded through the RP script to the IdP script.  % through \verb+postMessage+.
...
Even if an honest RP accepts a token which is generated for some malicious RP in another login and binds $PID_{U'} = [u't'r']G$ and $PID_{RP'}=[t'r']G$,
 ...
 due to the ECDLP,
  when user identities are kept secret (see Theorem \ref{thm-rp-designation}).
...
We discuss the impact of user identity leakage in Appendix \ref{sp-leak-uid}.
...
Thus, the IdP does not set \verb+redirect_uri+ in the HTTP responses, which is different from OIDC where HTTP redirections are used to implement these communications.
Most operations in the \emph{RP identity transformation} step take place within browsers,
