...
With the help of $t$, the RP derives a \emph{unique} account from different tokens for the same user in her different logins. This is the user's \emph{permanent} account at this RP. %, which is \emph{unique} among all accounts at the same RP. 
A user's accounts at different RPs are inherently different and unlinkable.
% (see Section \ref{sec:analysis} for detailed proofs).

A user's identity $u$ is kept secret from all entities except the honest IdP. Otherwise, colluding RPs could calculate $[u]ID_{RP_j}$s for any known $u$ and link these accounts.
...
We discuss the impact of leaked user identities in Appendix \ref{sp-leak-uid} with details.
