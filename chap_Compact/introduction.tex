...
\usso\ prevents %two privacy risks,
 both IdP-based login tracing and RP-based identity linkage, while existing privacy-preserving SSO solutions address only one of them \cite{BrowserID, SPRESSO, NIST2017draft, FirefoxAccount} or an extra trusted server is introduced \cite{miso}.
Meanwhile, \usso\ works compatibly with widely-used SSO protocols \cite{OpenIDConnect, rfc6749, SAML, NIST2017draft}, ensuring that all the desirable features of SSO services are supported.
