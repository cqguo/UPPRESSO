%定义所有的function
\newcommand{\funcPub}[1]{\mathsf{pub}(\mi{#1})}
\newcommand{\funcEncA}[1]{\mathsf{enc_a}(\mi{#1})}
\newcommand{\funcDecA}[1]{\mathsf{dec_a}(\mi{#1})}
\newcommand{\funcEncS}[1]{\mathsf{enc_s}(\mi{#1})}
\newcommand{\funcDecS}[1]{\mathsf{dec_s}(\mi{#1})}
\newcommand{\funcSig}[1]{\mathsf{sig}(\mi{#1})}
\newcommand{\funcCheckSig}[1]{\mathsf{checksig}(\mi{#1})}
\newcommand{\funcExtractMsg}[1]{\mathsf{extractmsg}(\mi{#1})}
\newcommand{\funcMultiply}[1]{\mathsf{multiply}(\mi{#1})}
\newcommand{\funcReverse}[1]{\mathsf{reverse}(\mi{#1})}
\newcommand{\funcCheckValid}[1]{\mathsf{checkvalid}(\mi{#1})}
...
\onecolumn
\input{chap_Compact/appendix_webmodel.tex}
...
\end{comment}
  ...
  \begin{algorithmic}[1]
    ...
    \myelse{path \equiv /startNegotiation}\label{line:rp-startNegotiation}
     ...
     %\mystate{\myss{t^{-1}:= \mathtt{Inverse}(t)}}
      ...
      \mystate{\myss{s'.\str{loginSessions}[\mi{loginSessionToken}] := \an{\mi{t}, \mi{PID_{rp}}, \mi{state}}}}\label{line:rp-store-t}
 ...
 In the following, we 
  ...
  \CASE{\myss{\str{expectT}}}
    \mystate{\myss{\mi{pattern} := \myangle{\tPostMessage, target, *, \myangle{\str{t}, *}}}}\label{line:rp-t-pattern}
    \mystate{\myss{\mi{input} := \textsf{CHOOSEINPUT}(\mi{scriptinputs}, \mi{pattern})}}\label{line:rp-t-input}
    \myif{\mi{input} \not\equiv \bot}
    \Let{$s'.\str{refXHR}$}{$\lambda_1$}
     % \mystate{\myss{t := \pi_4(\mi{input}).\str{t}}}\label{line:receive-t}
      ...
      \mystate{\myss{\mi{command} := \langle\tXMLHTTPRequest,\textsf{URL}^{\mi{RPDomain}}_\str{/startNegotiation},\mPost,\mi{body},}\breakalgohook{0}\myss{s'.\str{refXHR}\rangle}}\label{line:rp-t-send}
