...
For example, they could manipulate $PID_{RP}$ and $t$ in a login, attempting to (\emph{a}) entice honest users to return an identity token that could be accepted by some honest RP or (\emph{b}) manipulate the generation of $PID_U$ to analyze the relationship between $ID_U$ and $PID_U$.
...
This secret should be held \emph{only} by the user; otherwise, colluding adversaries could always link these accounts.
...
It implements secure SSO services against colluding users and RPs,
