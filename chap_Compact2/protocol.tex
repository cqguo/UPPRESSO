...
All entities except the honest IdP
...
Fortunately, $r$ is processed only \emph{once} by the IdP in UPPRESSO,
...
\begin{itemize}
  ...
  \item[2.]
  An identity token is forwarded to the target RP only,
    as authenticity and integrity of tokens have been ensured by the IdP's signatures
        and they are transmitted over HTTPS/TLS in UPPRESSO (i.e., the common mechanisms in widely-deployed SSO services),
\item[3.] 
$t$ is known only to the target RP and the initiating user (but not the IdP; otherwise, the IdP is able to infer $ID_{RP}$ from $PID_{RP}$ by itself),
and no direct privacy leakage in the requesting of identity tokens
(e.g., $ReEnpt_{RP}$ or $ID_{RP}$ is not included in the requests,
and no HTTP \texttt{referer} header about the RP server is leaked in HTTP requests to the IdP).
\end{itemize}
...
The \verb+postMessage+ targetOrigin mechanism \cite{postm-targeto} restricts the recipient,
    so the token is forwarded to exactly the target RP because (\emph{a}) only scripts downloaded from the origin of $ReEnpt_{RP}$ are legitimate recipients of the forwarded tokens
...
An RP certificate, denoted as $Cert_{RP}$, is introduced to bind $ReEnpt_{RP}$ and $ID_{RP}$, signed by the IdP during the RP's initial registration in UPPRESSO.
%
...
This HTTP request downloads the user-i script,
    which in turn verifies $Cert_{RP}$ to extract $ID_{RP}$ and $ReEnpt_{RP}$, and calculates $PID_{RP} = [t]ID_{RP}$. % that binds $ID_{RP}$ and $Enpt_{RP}$.
...
which honestly verifies $Cert_{RP}$ to ensure the correct relationship between $ReEnpt_{RP}$ and $ID_{RP}$.
This ensures that $PID_{RP} = [t]{ID_{RP}}$ is calculated based on the visited RP's identity
    ...
    because (\emph{a}) $ReEnpt_{RP}$ and $ID_{RP}$ are signed in $Cert_{RP}$ 
        and (\emph{b}) only scripts from the origin of $ReEnpt_{RP}$ are legitimate recipients of \verb+postMessage+.

In the HTTP request to pop up the user-i window, $Cert_{RP}$ and the scope of requested user attributes are carried and processed as \emph{fragments} (but not parameters) \cite{url-fragment}, which are identified by \texttt{\#} instead of \texttt{?},
    so that they are not sent to the IdP web server but processed by the browser.
Meanwhile, we save $t$ and $ReEnpt_{RP}$ using the \verb+sessionStorage+ HTML object \cite{htmlStorageHtml},
...
That is, $t$, $ReEnpt_{RP}$ and $Cert_{RP}$ are processed within browsers,
    not sent to the IdP.
...
Finally, 
two endpoints 
    specified in OIDC for the implicit login flow are implemented in an OIDC-compatible way \cite{rfc6749,OpenIDConnect}.
When receiving a token request for an ``unregistered'' $ID_{RP}$ (i.e., ephemeral $PID_{RP}$),
the IdP issues an identity token to an ``empty'' $ReEnpt_{RP}$,
    and the calculation of $PID_{U}$ is considered as a special way to assign PPIDs in OIDC.
The RP's redirection endpoint is also implemented compatibly,
    for $t$ is sent along with the tokens as an optional parameter.
...
locally maintains an \emph{up-to-date} list of all meaningful accounts at it,
...
Only if $Acct$ is meaningful in its local account list, the RP allows the user to log in.
...
However, it requires that the RP locally maintains an up-to-date account list,
