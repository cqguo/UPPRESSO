...
An RP certificate, denoted as $Cert_{RP}$ in this paper, is introduced to bind $ReEnpt_{RP}$ and $ID_{RP}$, signed by the IdP during the RP's initial registration.
%
...
Meanwhile, we save $ReEnpt_{RP}$ and $t$ using the \verb+sessionStorage+ HTML object \cite{sessionStorageHtml},
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=0.75\linewidth]{fig/pop-up-process.pdf}
...
The IdP rejects an identity-token request if the received $PID_{RP}$ is not a point on $\mathbb{E}$, and the RP rejects a token if it is not well signed or the derived account is meaningless. 
