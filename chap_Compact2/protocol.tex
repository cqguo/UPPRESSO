...
We assume that an RP with $ID_{RP_j}$
locally maintains an \emph{up-to-date} set of all meaningful accounts at it,
i.e., $\{Acct_i\}=\{\mathcal{F}_{Acct\ast}(ID_{U_i}, ID_{RP_j})\}$ for all registered users.
...
Only if $Acct$ is meaningful in its local account set, the RP allows the user to log in.
...
It requires that the RP locally maintains an up-to-date account set,
    to distinguish meaningless accounts from meaningful ones.
