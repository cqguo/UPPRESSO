...
Then, $Acct_{i,j} = \mathcal{F}_{Acct\ast}(ID_{U_i}, ID_{RP_j})= [ID_{U_i}]ID_{RP_j} =[u_ir_j]G$ is automatically assigned to a registered user at every RP.
...
it requires that the RP \emph{locally} maintains an up-to-date account set,
i.e., 
$\{Acct_i\}_j=\{[u_{i; 1 \leq i \leq s} r_j]G\}$ for all registered users and $|\{Acct_i\}_j|=s$.
