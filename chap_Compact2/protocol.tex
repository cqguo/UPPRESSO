...
%    \begin{tabular}{|c|c|c|}
   ...
   {$ID_{RP_j}$} & {$ID_{RP} = [r]G$ is the $j$-th RP's unique identity, which is publicly known; $r \in \mathbb{Z}_n$ is known to \emph{nobody}.} \\ \hline
    ...
    i.e., the RP's redirection endpoint \cite{rfc6749}.} \\ \hline
...
Moreover, we only improve the IdP's token endpoint in a way compatible with the original OIDC protocol \cite{rfc6749,OpenIDConnect}:
...
The IdP's other endpoints keep unchanged.
