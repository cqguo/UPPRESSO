...
In Section \ref{sec:analysis} we formally prove the other required properties (i.e.,
...
In UPPRESSO the IdP receives $PID_{RP}$ (but not $ID_{RP}$),
    so it cannot reply with $Enpt_{RP}$ to the user-i script.
%
...
when the user-i window is popped up,
    ...
    which is generated in the user-r window.
...
in the request to pop up the user-i window, $Cert_{RP}$ and the scope of requested user attributes are carried as \emph{fragments} (but not parameters), which are identified by \texttt{\#} instead of \texttt{?},
...
In UPPRESSO a user configures nothing locally for the IdP's public key is set in the user-i script to verify $Cert_{RP}$, like in other popular SSO systems \cite{OpenIDConnect, rfc6749, SAML, SAMLIdentifier}.
...
So, on receiving an identity-token request, the IdP web server checks the included HTTP \texttt{origin} header to ensure it is sent by the user-i script.
...
except that the relationship between $ID_{RP}$ and $Enpt_{RP}$ is maintained as $Cert_{RP}$ (i.e., in an offline way but still by the IdP).
Therefore, authenticity, confidentiality, and integrity of identity tokens are ensured in UPPRESSO.
Moreover, in UPPRESSO the IdP's token endpoint 
...
the IdP issuers a token but replies with an ``empty'' $Enpt_{RP}$,
...
Every RP regularly synchronizes all accounts at it from the IdP,
    and maintains an up-to-date local list of meaningful accounts.
...
The user-i script uses the \verb+sessionStorage+ HTML5 object to save $Enpt_{RP}$ and $t$,
    and then requests an identity token by sending $PID_{RP}$ and the scope of requested attributes.
\end{itemize}
...
\item[3.3] Only if it is meaningful in its account list, the RP allows the user to log in as $Acct$.
...
The IdP rejects an identity-token request in Step 3.3 if the received $PID_{RP}$ is not a point on $\mathbb{E}$, and the RP rejects the token holder if the identity token is invalid or the derived account is meaningless. 
