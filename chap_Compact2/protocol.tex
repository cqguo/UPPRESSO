...
%it is easy to shift these transformations into a finite field $\mathbb{F}_q$.
...
%    \centering
%    \begin{tabular}{|r|p{6.59cm}|} \hline
%    {\textbf{Notation}} & {\textbf{Description}} \\ \hline
%    {$\mathbb{E}$, $G$, $n$} & {$\mathbb{E}$ is an elliptic curve over a finite field $\mathbb{F}_q$. $G$ is a base point on $\mathbb{E}$, and the order of $G$ is a prime number $n$.} \\ \hline
...
%    {$PID_{U_i,j}^l$} & {$PID_U = [{ID_U}]{PID_{RP}} = [utr]G$ is the $i$-th user's pseudo-identity, in her $l$-th login visiting the $j$-th RP.} \\ \hline
...
%    {$ReEnpt_{RP}$} & {The RP's URL for receiving the identity tokens (or redirection endpoint \cite{rfc6749}).} \\ \hline
%    {$Cert_{RP}$} & {The RP certificate binding $ReEnpt_{RP}$ and $ID_{RP}$, signed by the IdP.} \\ \hline
%    \end{tabular}
%    \label{tbl:notations-protocol}
%\end{table}

\noindent {\bf $\boldsymbol{ID_{\boldsymbol{U}}}$, $\boldsymbol{ID_{\boldsymbol{RP}}}$ and $\boldsymbol{Acct}$.}
The IdP assigns a unique random integer $u \in \mathbb{Z}_n$ to a registered user (i.e., $ID_U = u$),
  and randomly selects unique $ID_{RP} = [r]G$ for a registered RP. % if it is a point on $\mathbb{E}$.
Here, %$G$ is a base point on $\mathbb{E}$, and
 $[r]G$ denotes the addition of $G$ on the curve $r$ times.
So $Acct_{i,j} = \mathcal{F}_{Acct\ast}(ID_{U_i}, ID_{RP_j})= [ID_{U_i}]ID_{RP_j} =[u_ir_j]G$ is automatically assigned to a registered user at every RP.
%A user's accounts at different RPs are inherently different and unlinkable.

All entities except the IdP
    know nothing about a user's identity $ID_U = u$; otherwise, colluding RPs could calculate $[u]ID_{RP_j}$ for any known $u$ and link these accounts.
This requires that $|\mathbb{Z}_n|$ is sufficiently large against exhaustive attacks; for example, $n$ is approximately $2^{256}$ for the NIST P256 elliptic curve and the amount of users is usually less than $2^{64}$.

$ID_{RP} = [r]G$ and $Acct = [ID_U]ID_{RP}$ are publicly-known,
...
In UPPRESSO $r$ is processed only \emph{once} by the IdP,
    while $u$ is used only by the IdP \emph{internally} and
...
After receiving an identity-token request for $PID_{RP}$ from a user authenticated as $ID_U$, the IdP calculates $PID_{U}$ as below.
\begin{equation}
...
First of all, the above designs ensure \emph{account uniqueness}:
    At an RP, every user owns a unique account, because $ID_{RP} = [r]G$ is also a generator of $\mathbb{E}$ and then
     $Acct = \mathcal{F}_{Acct\ast}()=[ur]G$ is unique locally at this RP.

In a login involving no adversary,
...
we formally prove the other required properties (i.e.,
    \emph{RP designation}, \emph{user identification}, \emph{IdP untraceability}, and \emph{RP unlinkability}) of the identity transformations under the adversarial scenarios.
...
\begin{itemize}
  \item[1.] $PID_{RP} = [t]{ID_{RP}}$ is calculated based on the target RP's identity but not any other RPs' (i.e., the target RP is designated),
  \item[2.]
  An identity token is forwarded to only the target RP web server in a browser,
    as authenticity and integrity of tokens have been ensured by the IdP's signatures
        and they are transmitted over HTTPS/TLS, % (i.e., the common mechanisms in widely-deployed SSO services),
\item[3.] 
$t$ is known only to the target RP and the initiating user (but not the IdP; otherwise, the IdP is able to infer $ID_{RP}$ from $PID_{RP}$),
and no direct privacy leakage in the requesting of identity tokens
(e.g., $ReEnptUrl_{RP}$ or $ID_{RP}$ is not included in the requests,
and no HTTP \texttt{referer} header about the RP web server is leaked in HTTP requests to the IdP).
\end{itemize}

In an SSO system implementing OIDC with pop-up UX, as shown in Figure \ref{fig:OpenID},
...
%Alternatively, 
    and the IdP replies with the signed token and also the RP's URL to receive tokens (i.e., $ReEnptUrl_{RP}$).
Then, after receiving the token, the user-i script forwards it to the user-r script by \verb+postMessage+
    which is restricted by the origin of $ReEnptUrl_{RP}$ \cite{SPRESSO,MITREid,BrowserID,GoogleIdIntegrate,de2014oauth,OpenIDConnect}.
The \verb+postMessage+ targetOrigin mechanism \cite{postm-targeto} restricts the recipient,
    so the token is forwarded to exactly the target RP because (\emph{a}) only scripts downloaded from the origin of $ReEnptUrl_{RP}$ are legitimate recipients of the forwarded tokens
    and (\emph{b}) the relationship between $ReEnptUrl_{RP}$ and $ID_{RP}$ is maintained at the honest IdP server.
...
The following improvements are proposed  in UPPRESSO,
    to satisfy the additional requirements in an OIDC-compatible way.
First of all, the IdP receives $PID_{RP}$ (but not $ID_{RP}$) in an identity-token request,
    so it cannot reply with $ReEnptUrl_{RP}$ to the user-i window.
An RP certificate, denoted as $Cert_{RP}$, is introduced to bind $ReEnptUrl_{RP}$ and $ID_{RP}$, signed by the IdP during the RP's initial registration.
%
...
when the user-i window is popped up,
    $Cert_{RP}$ and the scope of request user attributes are processed with the HTTP request to the IdP,
    all of which are generated by the user-r script downloaded from the target RP.
This HTTP request downloads the user-i script,
    which in turn verifies $Cert_{RP}$ to extract $ID_{RP}$ and $ReEnptUrl_{RP}$, and calculates $PID_{RP} = [t]ID_{RP}$. % that binds $ID_{RP}$ and $Enpt_{RP}$.
Then, after the IdP issues a token,
    the user-i script forwards it to the user-r script by \verb+postMessage+
    which is restricted by the origin of $ReEnptUrl_{RP}$.
Besides, on receiving an identity-token request, the IdP web server checks the included HTTP \texttt{origin} header to ensure it is sent by the user-i script,
which will honestly verify $Cert_{RP}$ to ensure the correct relationship between $ReEnptUrl_{RP}$ and $ID_{RP}$.
This ensures that $PID_{RP} = [t]{ID_{RP}}$ is calculated based on the visited RP's identity
    and only the designated RP receives the token,
    because (\emph{a}) $ReEnptUrl_{RP}$ and $ID_{RP}$ are signed in $Cert_{RP}$
        and (\emph{b}) only scripts from the origin of $ReEnptUrl_{RP}$ are legitimate recipients of \verb+postMessage+.

In the HTTP request to pop up the user-i window, $Cert_{RP}$ and the scope of requested user attributes are carried and processed as URL \emph{fragments} (but not query parameters) \cite{url-fragment}, which are identified by \texttt{\#} instead of \texttt{?},
    so that they are not sent to the IdP web server but locally processed by the browser.
Meanwhile, we save $t$ and $ReEnptUrl_{RP}$ using the \verb+sessionStorage+ HTML object \cite{htmlStorageHtml},
    because the user-i window may be refreshed to finish user authentication and attribute consent.
...
%    This HTTP request is the only communication from the user-r window to the IdP.
...
%Finally, authenticity, confidentiality, and integrity of identity tokens are still ensured in UPPRESSO.

%Compared with the original flow in Figure \ref{fig:OpenID}, UPPRESSO adopts the common mechanisms for secure communications, except that the relationship between $ID_{RP}$ and $ReEnpt_{RP}$ is maintained as $Cert_{RP}$ (i.e., in an offline way by the IdP).
Finally, 
two endpoints 
    specified in OIDC \cite{rfc6749,OpenIDConnect} for the implicit login flow are implemented in an OIDC-compatible way.
When receiving a token request for an ``unregistered'' $ID_{RP}$ (i.e., ephemeral $PID_{RP}$),
the IdP issues an identity token to an ``empty'' $ReEnptUrl_{RP}$,
    and the calculation of $PID_{U}$ is considered as a special way to assign PPIDs.
The RP's redirection endpoint is also implemented compatibly,
    where $t$ is sent along with the tokens as an optional parameter.
%Other endpoints specified in OIDC (e.g., userinfo endpoint and RP registration endpoint) are kept unchanged.
Moreover, the IdP's public key is set in the user-i script to verify $Cert_{RP}$,
so a user configures nothing locally,
 like in other OIDC implementations \cite{OpenIDConnect, GoogleIdIntegrate,de2014oauth,uber}.
...
\begin{figure*}[htb]
  \centering
  \includegraphics[width=0.762\linewidth]{fig/pop-up-process.pdf}
...
It submits a registration request, including the URL to receive identity tokens (i.e., $ReEnptUrl_{RP}$) and other information.
\item[2.]
...
The IdP then signs $Cert_{RP} = [ID_{RP}, ReEnptUrl_{RP}, *]_{SK}$,
...
The IdP assigns
a random but unique identity $ID_U = u\in \mathbb{Z}_n$ to the user.

$ID_U$ is known \emph{only} to the IdP
    and other entities know nothing about $ID_U$.
$ID_U$ is used only by the IdP \emph{internally} in UPPRESSO,
...
It verifies $Cert_{RP}$, extracts $ID_{RP}$ and $ReEnptUrl_{RP}$ from $Cert_{RP}$, 
chooses a random number $t \in \mathbb{Z}_n$,
    and calculates $PID_{RP}=[t]{ID_{RP}}$.
\item[1.3] 
Then, the user-i script saves $ReEnptUrl_{RP}$ and $t$ in the \verb+sessionStorage+ HTML5 object,
    and requests an identity token from the IdP by sending $PID_{RP}$ and the scope of requested attributes.
\end{itemize}
...
the user-i script loads $ReEnptUrl_{RP}$ and $t$, 
    and forwards the identity token and $t$ to the user-r window by \verb+postMessage+ which is restricted by the origin of $ReEnptUrl_{RP}$.
...
and then calculates $Acct = [t^{-1}]{PID_U}$.
Only if $Acct$ is meaningful, the RP allows the user to log in.
...
In Step 3.2 an RP does not check whether $PID_{RP}$ in the token is equal to $[t]ID_{RP}$ or not,
    and this efficient design does not result in attacks, which is proved in Section \ref{analysis-security}.

In order to distinguish meaningless accounts from meaningful ones in Step 3.2,
it requires that the RP \emph{locally} maintains an up-to-date account set for all registered users,
i.e., 
$\{Acct_i\}_j=\{[u_{i; 1 \leq i \leq s} r_j]G\}$ and $|\{Acct_i\}_j|=s$.
We discuss the means of account synchronization in Section \ref{account-syn}.
