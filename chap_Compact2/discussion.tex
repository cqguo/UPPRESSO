...
This work prefers to provide better user convenience by easy-to-deploy privacy-preserving designs,
rather than prevent IdP-RP collusive attacks.
Unless a user secret is involved in the derivation of accounts,
SSO cannot protect user privacy against an IdP colluding with RPs.
For example, even when the IdP strictly follows the protocols but shares information with RPs,
...
unless $Acct_{i,j} = \mathcal{F}_{Acct\ast}(s_i,ID_{U_i}, ID_{RP_j})$ is adopted and $s_i$ is a long-term user secret unknown to the colluding IdP and RPs.

As mentioned in Section \ref{subsec-solutions}, privacy-preserving identity federation \cite{ELPASSO, UnlimitID, idemix, PseudoID, uprov} prefers stronger privacy than user convenience,
    where a user secret is introduced to mask the relationship among the (pseudo-)identities and accounts.
In this case,
...
Moreover, if the user accesses the services from different devices,
    additional operations are required to synchronize the secret among these devices.
...
Then, in MISO the mixer server can track all users' login activities,
   either colluding with RPs or not.

%On the contrary,
%    UPPRESSO is designed to provide privacy-preserving SSO services accessed from COTS browsers without extra trusted servers. %      at the cost of user privacy vulnerable to IdP-RP collusive attacks.
...
Meanwhile, to hide RP identities from the IdP in the retrieval of identity tokens in the authorization code flow,
 ...
 and anonymous communications (e.g., Tor \cite{tor} or oblivious proxies \cite{ohttp-rfc}) need to be adopted for RPs;
...
\noindent \textbf{Support for redirect UX.}
With redirect UX, 
    an RP and the IdP exchange messages via HTTP redirection within one browser window.
If we implement the UPPRESSO protocol with redirect UX,
the following designs are needed to hide the visited RP's identity from the IdP % (i.e., to ensure IdP untraceability)
during these HTTP redirections.
These designs work either in the implicit flow of OIDC or the authorization code flow.

In an OIDC system without the protections of user privacy,
the RP's identity and redirection endpoint URL (i.e., $ID_{RP}$ and $ReEnptUrl_{RP}$) are sent as query parameters in the HTTP redirection to the IdP, 
    to request identity tokens.
On the contrary, in UPPRESSO
 when a user clicks and redirects the browser from the RP-login page to the IdP server,
    $ID_{RP}$ and $ReEnptUrl_{RP}$ are not sent.
Meanwhile,
    $Cert_{RP}$ and the scope of requested user attributes are carried as URL fragments,
so they are processed locally within the browser and not sent to the IdP web server.
Besides, the RP suppresses the HTTP \texttt{referer} header (e.g., by setting \verb+rel="noreferrer"+) of this redirection, which prevents the IdP server from receiving the RP's domain in the HTTP request of this redirection.

In the redirected page
the user-i script verifies $Cert_{RP}$, extracts $ID_{RP}$ and $ReEnptUrl_{RP}$,
calculates $PID_{RP}$, and then requests an identity token from the IdP's authorization endpoint.
Then, to forward the signed token to the RP,
    in the original OIDC protocol
        the IdP server constructs a server-side HTTP redirection to $ReEnptUrl_{RP}$.
However, in UPPRESSO the IdP does not know the visited RP,
    so it returns the signed token to the user-i script but with an empty $ReEnptUrl_{RP}$.
Then, the user-i script loads $ReEnptUrl_{RP}$ and $t$ locally from \verb+sessionStorage+, and submits the identity token and also $t$ to the RP's redirection endpoint 
by a client-side redirection (e.g., by \verb+window.location.href+).
...
%\vspace{0.75mm}
\noindent \textbf{Alternatives to generate $\boldsymbol{ID_{RP}}$ and bind $\boldsymbol{ReEnptUrl_{RP}}$.}
In UPPRESSO the IdP generates random $ID_{RP}$ and signs an RP certificate to bind $ID_{RP}$ and $ReEnptUrl_{RP}$, which is verified by the user-i script
    and then ensures the relationship between the RP designated by an identity token and the URL to receive this token.
It also guarantees that the target RP has already registered itself at the IdP and prevents unregistered RPs without such certificates from utilizing the IdP's services.

An alternative method to bind $ID_{RP}$ and $ReEnptUrl_{RP}$ is
 a deterministic scheme to calculate unique $ID_{RP}$ based on the RP's unambiguous name such as its domain.
...
In this case, in Step 1.2 the user-r script sends only the RP's redirection endpoint URL but not its RP certificate, and the user-i script calculates $ID_{RP}$ by itself based on the RP's domain. 
...
%\vspace{0.75mm}
\noindent \textbf{Restriction of the targetOrigin mechanism.}
The user-i script forwards tokens to the user-r script, and the \verb+postMessage+ targetOrigin mechanism \cite{postm-targeto} restricts the recipient of the forwarded identity tokens, to ensure that the tokens will be sent to the intended $ReEnptUrl_{RP}$, as specified in the RP certificate. The targetOrigin is specified as a combination of protocol, port (if not present, 80 for \verb+http+ and 443 for \verb+https+), and domain (e.g., \verb+RP.com+).
The origin of the user-r script must accurately match the targetOrigin to receive tokens.

The targetOrigin mechanism does not check the whole URL path in $ReEnptUrl_{RP}$, but it introduces no additional risk.
...
Because a browser controls access to web resources following the same-origin policy (SOP) \cite{sop},
    a user's resources in the honest RP web server is accessible to  the malicious server.
