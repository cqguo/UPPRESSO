...
In this work, we prefer to provide better user experience rather than prevent IdP-RP collusive attacks.
Even when the IdP strictly follows the protocols but shares information with RPs,
    ...
    unless a long-term secret, unknown to the colluding IdP and RPs,
                is used to mask the relationship among these (pseudo-)identities and accounts.
...
In this case, in Step 1.2 the user-r script sends only the RP's redirection endpoint (i.e., its URL for receiving tokens) but not its RP certificate, and the user-i script calculates $ID_{RP}$ by itself based on the RP's unambiguous name. 
