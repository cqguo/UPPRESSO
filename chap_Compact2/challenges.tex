...
In this section,
    we list the security requirements of SSO,
...
Existing privacy-preserving solutions of SSO hide only RP identities or user identities
    (i.e., only IdP-based login tracing or RP-based identity linkage is prevented),
        or introduce an extra fully-trusted component to process the (pseudo-)identities of both RP and user before an identity token is issued.
...
UPPRESSO \emph{simultaneously} transforms the identities of RP and user in each login,
        without extra trusted components more than the honest-but-curious IdP,
...
First of all, each registered user owns her account unique at an RP.
$\mathcal{F}_{Acct\ast}(ID_{U_i}, ID_{RP_j}) = Acct_{i,j}$ is proposed to determine a user's account at an RP.
An account is \emph{meaningful} at an RP if it belongs to some \emph{registered} user at this RP,
while a \emph{meaningless} account $A_m$ at $RP_j$ does not belong to any registered user, i.e., $\forall ID_{U_i} \in \mathbbm{ID}_U = \{ID_{U_{i; 1 \leq i \leq s}}\}: A_m \neq \mathcal{F}_{Acct\ast}(ID_{U_i}, ID_{RP_j})$.
That is, meaningless accounts belongs to ``potential'' users but not registered yet.
...
To ensure RP unlinkability (i.e., prevent RP-based identity linkage),
...
In addition to the basic requirement of account uniqueness,
    RP designation and user identification ensure security of SSO services,
