...
denoted as $ReEnpt_{RP}$), respectively.

User operations in the login flows are conducted on a user agent (or browser typically).
The flow with pop-up UX \cite{dimvaLiM16,GoogleIdIntegrate,uber} is proposed to provide an in-context user experience.
...
It pops up another window (i.e., the \emph{user-i window}),
    and this request is sent to the IdP's \emph{authorization endpoint} (i.e., the URL to issue tokens \cite{rfc6749,OpenIDConnect}).
...
After a token is issued by the IdP,
    it is sent to the RP's redirection endpoint as a fragment (but not parameters) of the URL due to security considerations,
so that a script from the RP is needed to handle this \cite{de2014oauth}.
...
\begin{table*}[tb]
  ...
  & & IdP-based Login Tracing & RP-based Identity Linkage & \textbf{Server}$^{\dag}$ & \textbf{User-maintained Secret}$^{\ddag}$ \\\hline
...
Existing SSO solutions propose various mechanisms to hide (\emph{a}) RP identities from the IdP and (\emph{b}) user identities from the colluding RPs.
...
Pairwise pseudonymous identifiers (PPIDs) are specified \cite{OpenIDConnect, SAMLIdentifier,NIST2017draft} to protect user privacy against colluding RPs.
...
\cite{ELPASSO, UnlimitID}
    and then the ``singleness'' property is broken,
while SSO aims to minimize a user's interactions with RPs.
Note that this user secret involved in the derivation of accounts is very different from the credential for user authentication to the IdP in SSO: If the credential is lost or leaked, the user  needs to renew it at the IdP only.}
...
In Opaak \cite{Opaak}, UnlimitID \cite{UnlimitID}, U-Prove \cite{uprov}, and EL PASSO \cite{ELPASSO}, the IdP signs anonymous credentials, each of which binds a user secret, %, with which users can authenticate to an RP.
...
Anonymous identity federation was formalized and constructed using cryptographic primitives (e.g., ZKP and group signature) \cite{WangWS13, HanCSTWW20, HanCSTW18}.
...
PrivacyPass \cite{privacypass} and TrustToken \cite{trusttoken} adopted the oblivious pseudo-random function (OPRF) protocol of Hash(EC)DH \cite{oprf-proved,voprf-proved} to generate anonymous tokens. %, with which a user ly accesses protected resources.
...
UPPRESSO employs a similar cryptographic technique (see Section \ref{subsec:overview} for details).\footnote{We designed and implemented UPPRESSO in 2020 \cite{uppresso-arxiv}, and in 2023 someone reminded us that the identity transformations in UPPRESSO are very similar to the Hash(EC)DH OPRF.} 
