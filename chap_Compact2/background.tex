...
\begin{table*}[tb]
  ...
  \multicolumn{2}{c|}{\textbf{Privacy Threat}$^{\sharp}$} & \textbf{Extra Trusted} & \textbf{Browser Extension} \\ \cline{3-4}
  & & IdP-based Login Tracing & RP-based Identity Linkage & \textbf{Server}$^{\dag}$ & \textbf{with User Secret}$^{\ddag}$ \\\hline
  ...
  \multirow{6}*{SSO} & OIDC w/ PPID \cite{NIST2017draft} & $\Circle$ & $\CIRCLE$ & $\CIRCLE$ & $\CIRCLE$ \\ \cline{2-6}
   ...
   & UP-SSO \cite{up-sso} & $\CIRCLE$ & $\CIRCLE$ & $\LEFTcircle$$^3$ & $\Circle$ \\ \hline 
