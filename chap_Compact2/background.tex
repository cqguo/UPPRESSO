...
\label{subsec:OIDC}
OIDC \cite{OpenIDConnect} is the most popular SSO protocol. It supports different login flows: implicit flow, authorization code flow, and hybrid flow (a mix of the other two).
...
and RP \emph{redirection endpoint URLs} to receive tokens (i.e., $ReEnptUrl_{RP}$), respectively  \cite{rfc6749}.
denoted as $ReEnptUrl_{RP}$, respectively  \cite{rfc6749}.

User operations in the login flows are conducted on a user agent (or browser typically).
The flow with pop-up UX is proposed to provide an in-context user experience \cite{dimvaLiM16,GoogleIdIntegrate,uber}.
As shown in Figure \ref{fig:OpenID},
when a user clicks the SSO login button in a browser window visiting the target RP 
(called the \emph{user-r window}),
 an identity-token request is constructed with the RP's identity, the RP's redirection endpoint URL, and the scope of requested user attributes.
It pops up another window (i.e., the \emph{user-i window}),
    and this request is sent to the IdP's \emph{authorization endpoint} where tokens are issued \cite{rfc6749,OpenIDConnect}.
After authenticating the user, the IdP issues an identity token that encloses the (pseudo-)identities of the authenticated user and the target RP, the user attributes, a validity period, etc.
...
After a token is issued by the IdP,
    it is sent to the RP's redirection endpoint as a fragment (but not query parameters) of the URL due to security considerations,
so a script from the RP is needed to handle this \cite{de2014oauth}.
The UPPRESSO prototype is built with pop-up UX,
    and we discuss the support for redirect UX in Section \ref{sec:discussion}.
...
\begin{table*}[tb]
  ...
  \multicolumn{2}{c|}{\textbf{Privacy Threat}$^{\sharp}$} & \textbf{Extra Trusted} & \textbf{Accounts Derived from} \\ \cline{3-4}
  & & IdP-based Login Tracing & RP-based Identity Linkage & \textbf{Component}$^{\dag}$ & \textbf{User Secret}$^{\ddag}$ \\\hline
...
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ ${\dag}$ \textbf{Extra Trusted Component}: $\CIRCLE$ no, $\Circle$ required.\\
${\ddag}$ \textbf{Accounts Derived from User Secret}: $\CIRCLE$ no, $\Circle$ yes.

\item[1.]
SPRESSO assumes a malicious IdP, so a trusted forwarder server is introduced to decrypt the RP identity and forward tokens to the RP.
...
Existing solutions propose various mechanisms to hide (\emph{a}) RP identities from the IdP and (\emph{b}) user identities from the colluding RPs.
Table \ref{tbl:comparison-protocol} compares these schemes,
and only UPPRESSO provides services accessed from COTS browsers while preventing both of the two privacy threats and not introducing extra trusted components in addition to the honest IdP.


Pairwise pseudonymous identifiers (PPIDs) are specified  to protect user privacy against colluding RPs \cite{OpenIDConnect, SAMLIdentifier,NIST2017draft}.
...
Other privacy-preserving schemes prevent IdP-based login tracing but leave users vulnerable to RP-based identity linkage, due to the identical user identities enclosed in identity tokens.
...
In SPRESSO an RP creates a one-time tag (i.e., ephemeral pseudo-identity) for each login \cite{SPRESSO},
 and in POIDC \cite{POIDC,save-flow} a user sends an identity-token request with a commitment on the target RP's identity (but not with plain $ID_{RP}$),
        which are enclosed in identity tokens along with $ID_U$.
...
MISO \cite{miso} decouples the assignment of PPIDs from an honest IdP,
...
It protects user privacy against even collusive attacks by the IdP and RPs,
    but the mixer could track a user's all login activities.
Similarly, UP-SSO \cite{up-sso} runs a fully-trusted Intel SGX enclave on the user side,
 which is remotely attested by the IdP and receives the secret to generate PPIDs. %for a user, and then both of two privacy threats are eliminated.
...
a long-term user secret is involved in the derivation of accounts,\footnote{Although the term ``single sign-on (SSO)'' was used in some schemes \cite{PseudoID, Opaak, ELPASSO, WangWS13, HanCSTW18, HanCSTWW20}, this paper refers to them as \emph{identity federation} to emphasize the difference.
Such a user-maintained secret used to derive accounts in identity federation harm the user convenience:
Once it is lost or leaked, the user has to notify all RPs to update her accounts derived from this secret %, or additional revocation checking will be needed 
    and then the ``singleness'' property is broken,
while SSO aims to minimize a user's interactions with RPs.
Note that this user secret involved in the derivation of accounts is very different from the credential for user authentication to the IdP: If this credential is lost or leaked in SSO, a user  renews it at the IdP only.}
 so browser extensions are required to maintain this user secret.
%In Section \ref{sec:discussion} we particularly discuss the prevention against IdP-RP collusive attacks by this secret and also the harm to user experience.



%In privacy-preserving identity federation this privacy protection depends on long-term user secrets to mask the relationship of among pseudonyms (or accounts), so a browser extension is always needed to maintain the user secrets. 


In PRIMA \cite{prima}, an IdP signs a credential
that binds user attributes and  a verification key.
Using the corresponding signing key, the user authenticates herself and provides selected attributes to RPs. The verification key works as the user's global identity and exposes her to RP-based identity linkage.
PseudoID \cite{PseudoID} introduces another trusted server in addition to the trusted IdP,
...
The user then unblinds this token,
    ...
    and then authenticates herself to an RP by this secret.
...
In Opaak \cite{Opaak}, UnlimitID \cite{UnlimitID}, U-Prove \cite{uprov}, and EL PASSO \cite{ELPASSO}, the IdP signs anonymous credentials, which bind a user secret, %, with which users can authenticate to an RP.
...
Some privacy-preserving solutions of identity federation \cite{PseudoID,ELPASSO,UnlimitID,Opaak,uprov,hyperledge-idemix} prevent both IdP-based login tracing and RP-based identity linkage, for (\emph{a}) the RP's identity is not enclosed in the anonymous credentials and (\emph{b}) the user selects different pseudonyms to visit different RPs.
...
Such approaches offer the strongest privacy,
    as a user visits RPs with pseudonyms that cannot be exploited to link any two actions.
Anonymous identity federation was formalized and constructed using cryptographic primitives (e.g., ZKP and group signature) \cite{HanCSTWW20, HanCSTW18, WangWS13}.
...
%
These completely-anonymous authentication services only work for special scenarios and do not support user identification (or account uniqueness) at each RP, a common requirement in most applications.

\subsection{OPRF-based Applications}
PrivacyPass \cite{privacypass} and TrustToken \cite{trusttoken} adopted the oblivious pseudo-random function (OPRF) of Hash(EC)DH \cite{oprf-proved,voprf-proved} to sign anonymous tokens. %, with which a user ly accesses protected resources.
A user generates a random number $e$ to blind an unsigned token $T$ into $[e]T$. After authenticating the user, a token server signs $[e]T$ using its private key $k$ and returns $[k e]T$. The user then uses $e$ to convert it into ($T, [k]T$),
        which is redeemed when she anonymously accesses resources.
...
UPPRESSO employs a similar cryptographic technique (see Section \ref{subsec:overview} for details).
...
similar to token signing by the token server in PrivacyPass/TrustToken.
IdP untraceability in UPPRESSO, i.e., the IdP cannot link $ID_{RP}$ and $[t]ID_{RP}$,
 roughly corresponds to unlinkability of token signing-redemption, i.e., the token server cannot link $T$ and $[e]T$  in PrivacyPass/TrustToken.

UPPRESSO leverages this cryptographic technique in very different ways.\footnote{We designed and implemented UPPRESSO in 2020 \cite{uppresso-arxiv}, and in 2023 someone reminded us that the identity transformations in UPPRESSO are very similar to the Hash(EC)DH OPRF.} 
First, 
...
In fact, UPPRESSO requires additional properties for secure SSO services 
    \cite{understanding-arxiv} (i.e., RP designation and user identification; see Theorems \ref{thm-u-id} and \ref{thm-rp-designation} in Section \ref{analysis-security}),
...
Thus, an OPRF is not always ready to work as the identity transformations in SSO.
