...
Then, the identity token issued by the IdP binds only $PID_U$ and $PID_{RP}$, instead of $ID_U$ and $ID_{RP}$. On receiving the token, % with a matching $PID_{RP}$,
 the target RP transforms $PID_U$ into an account \emph{unique at each RP but identical across multiple logins} visiting this RP.
...
UPPRESSO offers an easy-to-deploy option to enhance the protections of user privacy for SSO services, because no extra server is introduced and the services are accessed from commercial-off-the-shelf (COTS) browsers without extensions.
...
\item We designed the UPPRESSO protocol by integrating the identity transformations into OIDC compatibly, and proved the security or privacy guarantees of SSO services.
