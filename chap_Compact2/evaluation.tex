...
\begin{figure*}[tb]  % 星号*表示跨双栏
  ...
  \vspace*{-10pt}
  \par\medskip
  \centering{\footnotesize (a) In a local virtual environment}
...
Fortunately, once a browser visits a web server, it can cache the scripts locally, reducing the download time for subsequent visits. This caching is enabled by the \verb+localStorage+ HTML object \cite{htmlStorageHtml} and HTTP  \texttt{Cache-Control} header.
