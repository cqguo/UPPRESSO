...
$RP_j$ maintains an account set of all registered users,
i.e., $\{Acct_i\}=\{[u_{i; 1 \leq i \leq s} r_j]G\}$.
...
To distinguish meaningless accounts from meaningful ones in Step 3.2,
 an RP needs to regularly synchronize the accounts from the IdP to maintain an up-to-date set of meaningful accounts;
otherwise, 
  a \emph{newly-registered} user will be rejected
    because the derived account is \emph{falsely} considered as meaningless (i.e., not in the set).
...
On receiving an identity token from which a meaningless account is derived,
    an RP instantly contacts the IdP to receive the account set of users registered after the RP's last synchronization.
