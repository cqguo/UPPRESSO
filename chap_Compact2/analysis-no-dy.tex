...
\label{account-syn}
If new users are allowed to register after the system initialization,
in order to distinguish meaningless accounts from meaningful ones in Step 3.3,
 an RP regularly synchronizes its accounts from the IdP.
For example,
    when an account is derived but not in the visited RP's local account list,
        it contacts the IdP to synchronize accounts.
After this instant account synchronization, 
...
If an RP derives some account not in its local account list,
    it additionally checks $PID_{RP}$ in this case:
     If $PID_{RP}$ enclosed in the signed token is equal to $[t]ID_{RP}$,
                ...
                %\footnote{In this case, adversaries cannot manipulate $t$ or submit a token to RPs other than the designated one, and then the derived account is meaningful.}
                and then the RP updates its account list locally;
