...
In order to distinguish meaningless accounts from meaningful ones in Step 3.2,
 an RP needs to regularly synchronize the accounts at it to maintain an up-to-date list of meaningful accounts;
otherwise, 
  a \emph{newly-registered} user will be rejected
    because in her logins the derived account is falsely considered as meaningless (i.e., not in the visited RP's local account list).
...
On receiving an identity token from which a meaningless account is derived,
    an RP instantly contacts the IdP to receive the account list of users registered after the RP's last synchronization.
Then, after this instant account synchronization,\footnote{An RP does not straightforwardly treat the token holder with an account not in its local list
...
this malicious user would actually own another account $Acct=[mID_U]ID_{RP} \neq [ID_U]ID_{RP}$.} 
    the token holder will be rejected
     if the derived account is still considered as meaningless.
...
When an RP derives some account not in its local account list,
    it additionally checks $PID_{RP}$ in this case:
     If $PID_{RP}$ enclosed in the signed token is equal to $[t]ID_{RP}$,
                ...
                and then the RP locally updates its account list (i.e., this derived account is inserted into the list);
...
In this case, adversaries cannot (\emph{a}) manipulate $t$ to visit the designated RP because $t$ is checked in the equation of $PID_{RP} = [t]ID_{RP}$ 
 or (\emph{b}) submit a token to RPs other than the designated one
    because they cannot solve $t'$ satisfying that $[t']ID_{RP'} = [t]ID_{RP}$, which is equivalent to solve the ECDLP.
So the derived account must be meaningful when $PID_{RP}$ equals to $[t]ID_{RP}$.
