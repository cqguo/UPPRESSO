...
this section proves security (i.e., RP designation and user identification) and privacy (i.e., IdP untraceability and RP unlinkability) of the identity transformations in UPPRESSO.
...
First of all, security (i.e., RP designation and user identification) is proved with an honest IdP but against \emph{malicious RPs and users}.
...
Provided that the security properties are satisfied,
    we analyze the privacy properties \emph{only} for successful logins.
It makes no sense to track user activities
     from malicious behaviors deviating from the protocols, resulting unsuccessful logins.
So no malicious entities are considered in the proofs of privacy.
In particular, (\emph{a}) IdP untraceability is proved in Section \ref{subsec:IdP-privacy} against \emph{an honest-but-curious IdP}, which tries to infer the identities of the \emph{honest} RPs being visited by an \emph{honest} user,
...
In secure SSO systems, an identity token denoted as $TK$, which is requested by a user to visit an RP,
    \emph{enables only this user to log into only the honest target RP as her account at this RP} (i.e., RP designation and user identification). It makes no sense to discuss the login results at malicious RPs.
...
$Acct_{i,j}$ and $ID_{RP_j}$ are publicly-known, while $u_{i}$ and $r_{j}$ are kept secret to adversaries.
%An RP maintains its account set of all registered users, i.e., $\{Acct_i\}_j=\{[u_{i; 1 \leq i \leq s} r_j]G\}$ and $|\{Acct_i\}_j|=s$.
...
RP designation and user identification are proved in Theorems \ref{thm-rp-designation} and \ref{thm-u-id}, respectively, against malicious RPs colluding with users.
...
When given known $\mathbbm{ID}_{RP}$, known $\mathbbm{A}cct$ and unknown $\mathbbm{ID}_U$ defined as above,
...
where $PID_{RP}^{*}$ may be calculated as $\mathcal{F}_{PID_{RP}}(ID_{RP_{j_1}}, t_1)$
    (i.e., $RP_{j_1}$ is designated)
    or even arbitrarily generated (i.e., no RP is designated),
...
According to the designs and the detailed protocol in Sections \ref{sec:web-design} and \ref{implementations},
 it does not obtain any information about the target RP in a login (e.g., $ID_{RP}$, $ReEnptUrl_{RP}$ or $Cert_{RP}$), except the RP's ephemeral pseudo-identity $PID_{RP}$.

We prove that, \emph{$PID_{RP}$ is indistinguishable from a random variable on $\mathbb{E}$ to the IdP},
    and then
...
Given the trapdoor $t$, 
    adversaries can transform $PID_{RP}$ and $PID_U$ into $ID_{RP}$ and $Acct$, respectively, and vice versa.
...
In Theorem \ref{thm-rp-unlinkability} we prove that, 
...
to distinguish meaningless accounts from meaningful ones in Step 3.2,
 an RP needs to synchronize the accounts from the IdP to maintain an \emph{up-to-date} set of meaningful accounts;
otherwise, 
  a \emph{newly-registered} user will be rejected
    because the derived account is \emph{falsely} considered as meaningless (i.e., not in its local set).

Naive account synchronization may be finished as below.
On receiving an identity token from which a meaningless account is derived,
    an RP instantly contacts the IdP to retrieve the account set of users registered after the RP's last synchronization.
Then, after this instant account synchronization,\footnote{An RP does not straightforwardly treat the token holder with an account not in its local set
 as a newly-registered user;
otherwise,
although a malicious user cannot log into this RP as any meaningful account belonging to others according to Theorem \ref{thm-u-id},
 she could log into such an RP as an meaningless but identical account in multiple logins.
For example,
    by choosing ($\hat{t}_1$, $\check{t}_1$) and ($\hat{t}_2$, $\check{t}_2$) in two logins
     where $m=\hat{t}_1/\check{t}_1 = \hat{t}_2/\check{t}_2$,
    as analyzed in the proof of Theorem \ref{thm-u-id}.
this malicious user would actually own another account $Acct=[mID_U]ID_{RP} \neq [ID_U]ID_{RP}$.} 
    the token holder will be rejected
     if the derived account is still not in the updated set.
...
The following ``silient'' account synchronization
     leverages \emph{conditional} $PID_{RP}$ checking as below,
eliminating the communications in the ``active'' synchronization as above.
When an RP derives some account not in its local account set,
    it additionally checks $PID_{RP}$ in this case:
     If $PID_{RP}$ enclosed in the signed token is equal to $[t]ID_{RP}$,
                this account is asserted to belong to a newly-registered user (i.e., meaningful)
                and then the RP locally updates its account set (i.e., this derived account is inserted into the set);
    otherwise, it rejects this token binding $PID_{RP} \neq [t]ID_{RP}$.
In this case, adversaries cannot (\emph{a}) manipulate $t$ to visit the designated RP because $t$ is checked in the equation of $PID_{RP} = [t]ID_{RP}$ 
 or (\emph{b}) induce any RP other than the designated one to accept this token
    because they cannot solve $t'$ satisfying that $[t']ID_{RP'} = [t]ID_{RP}$, which is equivalent to solve the ECDLP.
So the derived account must be meaningful if $PID_{RP}$ is equal to $[t]ID_{RP}$.
