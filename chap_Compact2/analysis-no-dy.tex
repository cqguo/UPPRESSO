...
As mentioned in Section \ref{implementations}, an (honest) RP directly derives accounts based on \emph{any} signed identity tokens.
Such operations do not break user identification or RP designation,
    as proved in Theorems \ref{thm-u-id} and \ref{thm-rp-designation}.
The following proofs assume \emph{malicious RPs colluding with users},
...
\noarxiv{This property of obliviousness has been proved in the Hash(EC)DH OPRF \cite{oprf-proved,voprf-proved}, where the OPRF server works similarly to the IdP and learns nothing about a client's inputs or outputs of the evaluated pseudo-random function (i.e., $ID_{RP}$ or $Acct$ in UPPRESSO).}
...
in order to distinguish meaningful accounts from meaningless ones in Section 3.3,
