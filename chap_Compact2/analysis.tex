...
\vspace{1.5mm}
Next, we prove that \usso\ provides the ``RP Designation'' and ``User Identification'' properties that are used in the proof of Theorem \ref{thm-security}. Let us consider an identity token $TK$ binding $PID_{RP}$ and $PID_U = [ID_U]PID_{RP}$, which is generated by the IdP upon a request from an authenticated user with $ID_U$.


%\setcounter{lemma}{50}  % 使得相同Lemma的编号与Appendix一致
\begin{lemma}
\emph{A registered account $Acct = [u']ID_{RP}$ is derived at some honest RP with $ID_{RP}$ receiving $t$,
 from $TK$ binding $PID_{RP}$ and $PID_U = [ID_U]PID_{RP}$,
    \emph{if and only if} $PID_{RP} = [t]ID_{RP}$ and $ID_U = u'$.}
%with $ID_U$ at some honest RP or nothing.
% only the RP with $ID_{RP}$ and receiving $t$ in \usso, where $t \in [1,n)$, $ID_{RP} = [r]G$, $r$ is a unknown random number, and $G$ is a generator on $\mathbb{E}$ of order $n$.
...
\label{corollary-rp-designation}
\end{corollary}


\begin{corollary}
\textsc{(User Identification of $TK$)} \emph{$PID_U= [ID_U]PID_{RP}$ in $TK$ uniquely identifies an account at the RP designated by $PID_{RP} = [t]ID_{RP}$, and this account is uniquely mapped to a user with $ID_U$.}
\label{corollary-user-id}
\end{corollary}
