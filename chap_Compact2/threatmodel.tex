...
The system consists of an honest-but-curious IdP, as well as several honest or malicious RPs and users. This model is in line with widely-deployed SSO services in the Internet \cite{OpenIDConnect,rfc6749, SAML, SAMLIdentifier}.
...
It never actively violates the protocols,
 so a user-i script downloaded from the IdP also follows the protocols.
 % (see Section \ref{sec:web-design} for specific designs for web applications).

The IdP maintains its private key well for signing identity tokens and RP certificates, %(see Section \ref{implementations} for details)
...
Malicious RPs could behave arbitrarily, attempting to break the security or privacy guarantees for honest users and RPs \cite{SPRESSO, FettKS14,POIDC,miso}.
...
\noindent \textbf{Colluding RPs and users.}
Malicious users and RPs could collude,
 attempting to break the security or privacy guarantees;
that is,
their objective is to (\emph{a}) impersonate a victim user at honest RPs, (\emph{b}) entice an honest user to log into an honest RP under another user's account,
 and (\emph{c}) link an honest user's logins visiting colluding RPs.
For example, they could manipulate $PID_{RP}$ and $t$ in a login, attempting to (\emph{a}) entice honest users to return an identity token that could be accepted by some honest RP or (\emph{b}) affect the generation of $PID_U$ to analyze the relationship between $ID_U$ and $PID_U$.
...
These assumptions are realized by the commonly-used mechanisms in popular SSO services
 \cite{OpenIDConnect, rfc6749, SAML,GoogleIdIntegrate,de2014oauth,FettKS14,BrowserID,uber} (e.g., HTTPS/TLS, HTTP session maintenance, and the \verb+postMessage+ HTML5 API in COTS browsers; see Section \ref{sec:web-design} for specific designs for web applications).

As $PID_{RP}$ but not $ID_{RP}$ is processed by the IdP,
it also requires that no other information about the visited RP is leaked to the IdP,
 in the requesting, receiving and forwarding of identity tokens.
We realize this by specific designs in Section \ref{sec:web-design}.
Our work focuses on the privacy threats introduced by SSO protocols, and does not consider the tracking of user activities by network traffic analysis or crafted web pages, as they can be prevented by other defenses.
...
UPPRESSO is designed for users who value privacy,
        and provides an easy-to-deploy option for such users.
So privacy leakages due to re-identification by distinctive attributes across RPs are out of our scope.
A user never consents to enclose distinctive attributes, such as telephone number, Email address, etc., in tokens or sets such attributes at any RP.
